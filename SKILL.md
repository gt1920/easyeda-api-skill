...
- **★ macOS 上给 EDA 开第二个窗口**:`open -n -a "嘉立创EDA(专业版)"` 触发 Chromium second-instance,
  第一实例会弹出一个**新窗口**(开始页),配合多实例桥接(先手动起 `BRIDGE_PORT=4962x node bridge-server.mjs`),
  新窗口的扩展自动认领下一个空闲端口——两个会话各占一窗一桥互不干扰。菜单里没有"新窗口"入口,UI 右键也难做
...
- **一个位号在多台/多窗并行时的车辙**:动手前先 `getCurrentProjectInfo` 核对当前窗口的工程——
  ...
  create 超时且因活动文档是 PCB 页而实际未执行,零损伤纯属侥幸)。桥接端口=会话边界,别共用。
